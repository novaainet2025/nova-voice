...
| 항목 | 내용 | 상태 |
|------|------|------|
| 파일 드래그→Claude | V2T/터미널 모드 무관하게 항상 Claude로 전달, PTY 우선/claude-p 폴백 | ✅ |
| TTS 화자 미리듣기 버그 | 화자 카드 ▶가 항상 :7860 다화자 서버 사용 (모든 화자 구분 가능) | ✅ |
| mlx 단일화자 경고 | mlx/mlx_ko 모드에서 노란 경고 배너 표시 | ✅ |
| 명령어 자연어 표현 | 터미널 명령어 대신 자연어로 행위 설명 | ✅ |
