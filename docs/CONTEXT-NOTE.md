...
6. **MLX-Qwen3(:8800) 단일화자**: Qwen3-TTS-Base는 화자 선택 미지원 — 실제 출력은 항상 동일 목소리. 화자 선택은 :7860 Qwen3-TTS 모드에서만 유효
...
### 파일 드래그 → Claude 직접 전달
- `UnifiedPanel.tsx` — V2T/AI Terminal 모드 관계없이 드래그된 파일을 항상 Claude로 전달
- PTY 준비 시 `ptyType`으로 인터랙티브 세션 직접 타이핑, 없으면 `claude -p` 폴백
- 메시지 포맷 개선: "파일을 첨부합니다" → "이 파일을 읽고 분석해줘: ${path}"
- 파일 드래그 시 자동으로 AI Terminal 탭 전환
- 이미지는 첨부 대기열 유지 (Claude PTY는 텍스트 전용)

### TTS 화자 미리듣기 버그 수정
- `SettingsPanel.tsx` — 화자 카드 ▶ 미리듣기를 항상 :7860 Qwen3-TTS(다화자)로 라우팅
  - 기존: mlx_ko/mlx 모드에서 :8800 단일화자 → 모든 화자가 동일한 소리
  - 수정: 카드 미리듣기는 ttsModel과 무관하게 항상 :7860에서 8화자 구분 재생
- mlx/mlx_ko 모드 선택 시 노란 경고 배너 추가: "단일화자 모델 — 실제 출력은 모두 동일"
